            ...
            try {
                ...
                if (type === "nodes") {
                    promise = new Promise(resolve => {
                        setTimeout( () => resolve(loadNodeConfig(things[thingName])), 2000)
                    });
...
if (false) {
    const allPromises = nodePromises.concat(pluginPromises);
    return Promise.all(allPromises).then(function(results) {
        const allList = results.filter(r => !!r);
        // Initial load has happened. Ensure all modules are in the registry now.
        for (var module in modules) {
            if (modules.hasOwnProperty(module)) {
        ...
        return loadNodeSetList(allList);
    })
} else {
    var pluginList;
    var nodeList;
...
async function loadNodeConfig(fileInfo) {
    ...
    console.log("loadNodeConfig", file)
